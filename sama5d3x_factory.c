...
static void do_test(int automatic)
{
	...
	while (1) {
...
#if 0			
			printf("testing eth0...\n");
			if ((ret = eth_test("eth0", "192.168.2.233", "192.168.2.128")) != 0)
				err_on_testing = 1;
		...
		case 6:
...
#endif
			system("/root/mount_usb.sh mmcblk0p1");
			...
			system("/root/mount_usb.sh mmcblk1p1");
			if (ret = storage_test("/media/mmcblk1p1", "MYiR_test.txt") < 0)
				err_on_testing = 1;
			if (!automatic) {
				printf(" . %-15s %s\n", "mmcblk1p1", ret == 0 ? "ok" : "err");
				break;
			} else {
				sprintf(result_list[result_idx++],
						" . %-15s %s", "mmcblk1p1", ret == 0 ? "ok" : "err");
			}

		case 7:
			printf("testing mtd...\n");
			if (ret = mtd_test("/dev/mtd1", "Test SPI Flash, MYiR") < 0)
				err_on_testing = 1;
